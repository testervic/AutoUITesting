...
//    }
    @Test(priority = 1)
    public void SqProductList() throws YamlException, FileNotFoundException, InterruptedException {
        LoginPage loginPage = new LoginPage(this.driver, "/Login.yaml");
        loginPage.operate();
        LoginPage sqProductList = new LoginPage(this.driver, "/SqProductList.yaml");
        sqProductList.operate();
        Assert.assertTrue(sqProductList.checkpoint(this.browserType), "检查点不通过");
